...
static ma_result read_pcm_frames(ma_data_source* pDataSource,
                                 void* pFramesOut,
...
static ma_result get_data_format(ma_data_source* pDataSource,
                                 ma_format* pFormat,
                                 ma_uint32* pChannels,
    ...
    auto* s = self(pDataSource);
    return ma_data_source_get_data_format(s->upstream, pFormat, pChannels, pSampleRate,
...
ma_result dsp_data_source_init(DSPDataSource* s,
    ...
    ma_result r = ma_data_source_get_data_format(upstream, &format, &channels, &sampleRate,
                                                 nullptr, 0);
    if (r != MA_SUCCESS) return r;
