...
interface NativeDSP {
  ...
  setCompressor(c: CompressorConfig): void;
  clearCompressor(): void;
  setSoftClip(o: { enabled: boolean; drive: number }): void;
  resetState(): void;
  reset(): void;
  destroy(): void;
}
interface NativeAddon {
  version(): string;
  createDSP(o: DSPOptions): NativeDSP;
}
interface NativeDecoderAddon {
  NativeDecoder: new (input: Buffer | string) => NativeDecoder;
}
function loadNative(): NativeAddon {
  return require("../build/Release/audio_dsp.node") as NativeAddon;
}
function loadDecoder(): NativeDecoderAddon {
  return require("../build/Release/audio_decoder.node") as NativeDecoderAddon;
}
export function createDecoder(input: Buffer | string): NativeDecoder {
  if (!Buffer.isBuffer(input) && typeof input !== "string")
    throw new TypeError(
      "createDecoder() requires an encoded Buffer or file path",
    );
...
function validateOptions(o: DSPOptions) {
...
function validateBiquad(c: BiquadConfig, sr: number) {
  if (!c || !BT.includes(c.type)) throw new TypeError("invalid biquad type");
  if (
...
function validateEQ(b: EQBand[], sr: number) {
...
function validLimiter(c: LimiterConfig) {
  const t = c.threshold ?? -1,
    r = c.release ?? 50;
  if (!Number.isFinite(t) || t > 0 || t < -24 || !Number.isFinite(r) || r <= 0)
    throw new RangeError("invalid limiter parameters");
}
function validComp(c: CompressorConfig) {
  const t = c.threshold ?? -18,
    r = c.ratio ?? 4,
    a = c.attack ?? 10,
    rel = c.release ?? 100;
  if (
...
export function createDSP(o: DSPOptions): AudioDSP {
  validateOptions(o);
  const n = loadNative().createDSP(o);
  let dead = false;
  const ids = new Set<string>();
  const guard = () => {
    if (dead) throw new Error("AudioDSP instance has been destroyed");
  };
  return {
    process(i) {
      guard();
    ...
    setMute(v) {
      guard();
      if (typeof v !== "boolean") throw new TypeError("muted must be boolean");
      n.setMute(v);
    },
    setPan(v) {
      guard();
      ...
      if (o.channels < 2) throw new Error("pan requires at least 2 channels");
      n.setPan(v);
    },
    setBiquad(c) {
      guard();
    ...
    addBiquad(id, c) {
      guard();
      validId(id);
      if (ids.has(id)) throw new Error(`filter id already exists: ${id}`);
      validateBiquad(c, o.sampleRate);
      n.addBiquad(id, c);
      ids.add(id);
    },
    ...
    removeFilter(id) {
      guard();
      validId(id);
      if (!ids.has(id)) throw new Error(`unknown filter id: ${id}`);
      n.removeFilter(id);
      ids.delete(id);
    },
